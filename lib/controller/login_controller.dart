...
import '../view/login_signup/accout_created_screen.dart';
...
class LoginController extends GetxController {
      ...
      try {
        ...
        if (currentUser != null) {
          trusterUser.value = TrusterUser(
              ...
              countryCode: CountryCode.getCountryCodeFromString(selectedCountryCode.value),
              imageUrl: '',
              ssnNumber: '',
  ...
  Future<void> addUserData() async {
    await FirebaseFirestore.instance.collection(usersCollection).doc(currentUser!.uid).set(trusterUser.value!.toJson());
  }
  ...
  Future<bool> getUserData() async {
    ...
    try {
      final String? fcmToken = await FirebaseMessaging.instance.getToken();
      await FirebaseFirestore.instance.collection(usersCollection).doc(currentUser!.uid).get().then((snapShot) {
      ...
      if (trusterUser.value!.fcmToken != fcmToken!) {
        ...
        await FirebaseFirestore.instance.collection(usersCollection).doc(currentUser!.uid).update({'fcmToken': fcmToken});
        await FirebaseFirestore.instance.collection(contactsCollection).doc(currentUser!.uid).update({'fcmToken': fcmToken});
      }
    } catch (e) {
      logoutUser();
    }
  ...
  Future<void> fcmTokenListener() async {
    FirebaseMessaging.instance.onTokenRefresh.listen((fcmToken) {
      FirebaseFirestore.instance.collection(usersCollection).doc(currentUser!.uid).update({'fcmToken': fcmToken});
      FirebaseFirestore.instance.collection(contactsCollection).doc(currentUser!.uid).update({'fcmToken': fcmToken});
    });
  }
